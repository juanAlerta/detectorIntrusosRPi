...
def capture(path: Path) -> bool:
    ...
    result = subprocess.run(
        [
            ...
            "--no-banner",       # sin barra de fecha en la imagen
            "--skip", "2",       # descarta los primeros 2 frames (auto-exposición)
            "--quiet",
            str(path),
        ],
        capture_output=True,
...
def send_alert(bot: telegram.Bot, image_path: Path) -> None:
...
def main() -> None:
    ...
    # Verificar conexión con Telegram al arrancar
    try:
        me = bot.get_me()
        log.info("Bot conectado: @%s", me.username)
        bot.send_message(chat_id=CHAT_ID, text="✅ DetectorIntrusos activo y vigilando.")
    except telegram.error.TelegramError as e:
        log.error("No se pudo conectar con Telegram: %s", e)
        raise SystemExit(1)

    prev_frame = None
    cooldown   = 0          # segundos de espera tras una alerta para no saturar
    COOLDOWN_S = 30         # no enviar más de una alerta cada 30 segundos
        ...
        try:
            ...
            if prev_frame is not None:
                if cooldown <= 0 and detect_motion(prev_frame, curr_frame):
                    ...
                    # Guardar copia de la alerta antes de que SNAP_PATH se sobreescriba
                    import shutil
                    shutil.copy(SNAP_PATH, ALERT_PATH)
                    send_alert(bot, ALERT_PATH)
                    cooldown = COOLDOWN_S
                elif cooldown > 0:
                    cooldown -= INTERVAL
